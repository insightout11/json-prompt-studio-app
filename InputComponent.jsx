...
const InputComponent = ({ className = "", aiFeatures = null, resetTrigger }) => {
  console.log('🚨🚨🚨 InputComponent - COMPLETELY NEW COMPONENT NAME - NO CACHE 🚨🚨🚨');
  ...
  useEffect(() => {
    console.log('🎯 InputComponent resetTrigger changed:', resetTrigger);
    if (resetTrigger) {
  ...
  const handleTextToJson = async () => {
    ...
    try {
      console.log('🚨🚨🚨 InputComponent making DIRECT API call - BYPASSING ALL CACHE');
      ...
      const response = await fetch('/api/groq', {
        ...
        body: JSON.stringify({
          messages: [{
            role: 'user',
            content: `Convert this scene description into structured JSON. Return ONLY valid JSON.

Scene: "${textInput}"

Fields: scene, character_type, setting, actions, emotions, lighting_type, time_of_day, camera_angle, camera_distance, style, color_palette, atmosphere, clothing, hair_color, hair_style, age, gender, environment.
...
Return ONLY valid JSON with relevant fields.`
      ...
      console.log('🔥 InputComponent API status:', response.status);
      
      if (!response.ok) {
        const errorText = await response.text();
        console.error('🚨 InputComponent API error:', errorText);
        throw new Error(`API failed: ${response.status}`);
      }
      
      const responseText = await response.text();
      console.log('🔥 InputComponent response length:', responseText.length);
      console.log('🔥 InputComponent response preview:', responseText.substring(0, 200));
      
      const apiData = JSON.parse(responseText);
      const aiContent = apiData.choices[0].message.content;
      console.log('🔥 InputComponent AI content:', aiContent);
      ...
      console.log('🔍 InputComponent final JSON:', cleanedResponse);
      ...
      console.error('🚨 InputComponent error:', err);
  ...
  const handleTextEnhancement = async () => {
    ...
    try {
      console.log('🚨🚨🚨 InputComponent ENHANCEMENT call - DIRECT API');
      ...
      const response = await fetch('/api/groq', {
        ...
        body: JSON.stringify({
          messages: [
            { role: 'system', content: 'Enhance creative content with rich details.' },
            { role: 'user', content: `Enhance these fields with more detail:\n\n${currentFields}\n\nReturn enhanced JSON with richer descriptions.` }
          ],
          temperature: 0.8,
          max_tokens: 1000,
          model: 'llama-3.1-8b-instant'
      ...
      if (!response.ok) throw new Error(`Enhancement failed: ${response.status}`);
      ...
      console.error('🚨 InputComponent enhancement error:', err);
