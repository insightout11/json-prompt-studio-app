...
const UniversalInput = ({ className = "", aiFeatures = null, resetTrigger }) => {
  console.log('🚨🚨🚨 UniversalInput component loaded - v8.0 FULLY DIRECT API - NO CACHE 🚨🚨🚨');
...
Return ONLY valid JSON with fields you're confident about. Use descriptive but concise values.`;
      ...
      console.log('🔥 UniversalInput making DIRECT API call for text-to-json...');
      ...
      console.log('🔥 Direct API data received:', directData);
      console.log('🔍 Choices array exists:', !!directData.choices);
      console.log('🔍 First choice exists:', !!directData.choices?.[0]);
      console.log('🔍 Message exists:', !!directData.choices?.[0]?.message);
      console.log('🔍 Content exists:', !!directData.choices?.[0]?.message?.content);
      
      if (!directData.choices?.[0]?.message?.content) {
        console.error('🚨 Invalid response structure:', directData);
...
Return enhanced JSON with richer, more detailed descriptions. Don't remove existing content - build upon it.`;
      ...
      console.log('🔥 UniversalInput making DIRECT API call to bypass caching...');
      
      // Direct API call to test if endpoint is working
      const directResponse = await fetch('/api/groq', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
      ...
      console.log('🔥 UniversalInput received response:', response);
      ...
      console.log('🔥 Response content length:', response?.content?.length || 'undefined');
      if (response?.content) {
        console.log('🔥 Response content first 50 chars:', JSON.stringify(response.content.substring(0, 50)));
      }
      ...
      try {
        ...
        console.log('🔧 Cleaning response for JSON issues...');
        
        // Fix common JSON issues that Groq generates
        let repairedResponse = cleanedResponse
          // Fix unescaped quotes in strings
          .replace(/:\s*"([^"]*?)"([^",:}]*?)"([^"]*?)"/g, (match, start, middle, end) => {
            const escapedMiddle = middle.replace(/"/g, '\\"');
            return `: "${start}\\"${escapedMiddle}\\"${end}"`;
          })
          // Fix trailing commas
          .replace(/,(\s*[}\]])/g, '$1')
        ...
        jsonData = JSON.parse(repairedResponse);
      } catch (parseError) {
        console.error('JSON parsing error:', parseError);
        console.error('🚨 Malformed JSON content:', cleanedResponse);
  ...
  const handleImageToJson = async () => {
    ...
    try {
      ...
      let result = { success: false, fields: {} };
        ...
        try {
          ...
          if (jsonMatch) {
            ...
            if (parsedResult.fields) {
              result = { success: true, fields: parsedResult.fields };
            }
          }
        } catch (parseErr) {
          console.error('Failed to parse AI response:', parseErr);
          result = { success: false, error: 'Failed to parse AI analysis.' };
        }
      }
