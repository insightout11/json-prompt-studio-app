...
const PreviewTray = ({ 
    ...
    try {
      ...
      if (data.status === 'completed') {
        ...
        if (data.images && data.images.length > 0) {
          ...
          addToast(`Generated ${newImages.length} image${newImages.length > 1 ? 's' : ''}!`, 'success');
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      addToast(`Generation started! ${data.estimatedWait}`, 'info');
  ...
  const handleUseInStoryboard = async (image) => {
    if (!storyboardSlotId) {
      addToast('Use in Storyboard: Please select a scene first', 'info');
      return;
    }
